...
let displayQuestions = async (newDiv) => {
    ...
    try {
        ...
        if (OPENAI_API_KEY == undefined) {
            ...
            newDiv.innerHTML += "3. Paste your API key below and click 'OK'.<br><br>";

            let input = document.createElement('input');
            input.style.padding = "4px";
            input.placeholder = "Your API key";
            newDiv.appendChild(input);
            registeredElements.push(input);
            ...
            error.innerHTML = "Invalid API key"
            error.style.marginTop = "15px";
            error.style.color = "#F00";
            error.style.fontWeight = "500";
            error.style.display = "none"
            newDiv.appendChild(error);
            registeredElements.push(error);
